...
static void draw_quad_rotate(GCanvas* canvas) {
    ...
    GPathBuilder bu;
    bu.moveTo(pts[0]);
    bu.quadTo(pts[1], pts[2]);
    bu.quadTo(pts[3], pts[0]);
    auto path = bu.detach();
...
static void draw_rings(GCanvas* canvas) {
    ...
    bu.addCircle({256, 256}, 200, GPathDirection::kCW);
    bu.addCircle({256, 256}, 160, GPathDirection::kCCW);
    ...
    bu.addCircle({100, 100}, 90, GPathDirection::kCCW);
    ...
    bu.lineTo(r.right, r.bottom);
    bu.lineTo(r.right, r.top);
    bu.addCircle({100, 100}, 90, GPathDirection::kCW);
